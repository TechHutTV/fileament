...
Share links are read-only and scoped to one model or collection. They can expire after a chosen number of days or remain active without an expiration date, and they can be revoked at any time. **Settings → Share links** shows each target, creation and expiration dates, lifecycle status, copy-ready public URL, and shared-page view count. A view is one successful shared-page load; thumbnail, mesh, image, file, pagination, and background refresh requests do not inflate it. Visible shared pages refresh metadata once per minute and recheck access on focus or reconnect. Collection refreshes restart from the first page and follow current cursors through the number of pages already loaded, requesting one page at a time. Fresh pages replace stale members as they arrive; a failed refresh preserves the latest successful data and offers a retry. Removing the selected member clears its details and downloads while leaving the collection available. Expiration or confirmed revocation hides the share, including any open viewer. Leaving the page cancels its requests and clears its metadata; viewer approval belongs to the selected file and share. Shared pages are marked `noindex` for search engines.
...
Model downloads, viewer meshes, images, and thumbnails are served from regular files beneath the configured data directory. Asset requests return `404` for symbolic links, directories, and special files, including links in a model's parent directories. The configured data directory itself may be a symbolic link. Owner authentication and public share scope checks apply before opening an asset.

Before recovery or database initialization, startup rejects symbolic links and special files anywhere in persistent storage. Resolve them while Fileament is stopped, preserving their contents as ordinary files or directories. Disposable `/data/tmp` contents are cleared without following links. This startup check does not permit external tools to modify storage while Fileament is running.
...
| `GET /api/public/{token}` | For a model share, returns `{share, model}`. For a collection share, returns `{share, collection, model}`: a paginated collection summary plus one complete selected model. Use `model={id}` to select a current member, including one outside the current page; otherwise the first member of the page is selected. An empty collection has `model: null`. Supports `limit` and `cursor` for collection pages. Add `refresh=1` for background reads and pagination without incrementing the view count. In refresh mode, an unavailable selected member returns the valid collection with `model: null` and `modelUnavailable: true`; token and target checks still apply. |
